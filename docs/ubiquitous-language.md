...
| `Note catalog row`       | The lightweight list representation of a Note used by the Workspace Catalog. It keeps the same top-level fields as a Note, but its `content` field contains only the preview slice used by `NotesListPanel`, not the full Content.                                         | note preview, list item payload  | partial note, cached note                         |
