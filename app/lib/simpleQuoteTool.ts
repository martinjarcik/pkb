import type {
  API,
  ConversionConfig,
  HTMLPasteEvent,
  PasteConfig,
  SanitizerConfig,
  ToolConfig,
  ToolboxConfig,
} from '@editorjs/editorjs'

const ICON_QUOTE =
  '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 10.8182L9 10.8182C8.80222 10.8182 8.60888 10.7649 8.44443 10.665C8.27998 10.5651 8.15181 10.4231 8.07612 10.257C8.00043 10.0909 7.98063 9.90808 8.01922 9.73174C8.0578 9.55539 8.15304 9.39341 8.29289 9.26627C8.43275 9.13913 8.61093 9.05255 8.80491 9.01747C8.99889 8.98239 9.19996 9.00039 9.38268 9.0692C9.56541 9.13801 9.72159 9.25453 9.83147 9.40403C9.94135 9.55353 10 9.72929 10 9.90909L10 12.1818C10 12.664 9.78929 13.1265 9.41421 13.4675C9.03914 13.8084 8.53043 14 8 14"/><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 10.8182L15 10.8182C14.8022 10.8182 14.6089 10.7649 14.4444 10.665C14.28 10.5651 14.1518 10.4231 14.0761 10.257C14.0004 10.0909 13.9806 9.90808 14.0192 9.73174C14.0578 9.55539 14.153 9.39341 14.2929 9.26627C14.4327 9.13913 14.6109 9.05255 14.8049 9.01747C14.9989 8.98239 15.2 9.00039 15.3827 9.0692C15.5654 9.13801 15.7216 9.25453 15.8315 9.40403C15.9414 9.55353 16 9.72929 16 9.90909L16 12.1818C16 12.664 15.7893 13.1265 15.4142 13.4675C15.0391 13.8084 14.5304 14 14 14"/></svg>'

type SimpleQuoteConfig = ToolConfig & {
  placeholder?: string
  preserveBlank?: boolean
}

type SimpleQuoteData = {
  text: string
}

type SimpleQuoteParams = {
  api: API
  config: SimpleQuoteConfig
  data: SimpleQuoteData
  readOnly: boolean
}

type SimpleQuoteCss = {
  block: string
  wrapper: string
}

function makeFragment(htmlString: string): DocumentFragment {
  const tempDiv = document.createElement('div')

  tempDiv.innerHTML = htmlString.trim()

  const fragment = document.createDocumentFragment()

  fragment.append(...Array.from(tempDiv.childNodes))

  return fragment
}

export function normalizeSimpleQuoteText(text: string): string {
  return text
    .replace(/\r\n|\r|\n/g, ' ')
    .replace(/<br\b[^>]*\/?>/gi, ' ')
    .trim()
}

export default class SimpleQuoteTool {
  static get DEFAULT_PLACEHOLDER(): string {
    return ''
  }

  static get conversionConfig(): ConversionConfig {
    return {
      export: 'text',
      import: 'text',
    }
  }

  static get sanitize(): SanitizerConfig {
    return {
      text: {
        br: true,
      },
    }
  }

  static get isReadOnlySupported(): boolean {
    return true
  }

  static get pasteConfig(): PasteConfig {
    return {
      tags: ['BLOCKQUOTE'],
    }
  }

  static get toolbox(): ToolboxConfig {
    return {
      icon: ICON_QUOTE,
      title: 'Simple Quote',
    }
  }

  api: API
  readOnly: boolean

  private css: SimpleQuoteCss
  private placeholder: string
  private data: SimpleQuoteData
  private element: HTMLDivElement | null
  private preserveBlank: boolean

  constructor({ api, config, data, readOnly }: SimpleQuoteParams) {
    this.api = api
    this.readOnly = readOnly
    this.css = {
      block: this.api.styles.block,
      wrapper: 'ce-simple-quote',
    }

    if (!this.readOnly) {
      this.onKeyUp = this.onKeyUp.bind(this)
    }

    this.placeholder = config.placeholder ?? SimpleQuoteTool.DEFAULT_PLACEHOLDER
    this.data = {
      text: normalizeSimpleQuoteText(data?.text ?? ''),
    }
    this.element = null
    this.preserveBlank = config.preserveBlank ?? false
  }

  onKeyUp(event: KeyboardEvent): void {
    if (event.code !== 'Backspace' && event.code !== 'Delete') {
      return
    }

    if (!this.element) {
      return
    }

    if (this.element.textContent === '') {
      this.element.innerHTML = ''
    }
  }

  render(): HTMLDivElement {
    this.element = document.createElement('div')
    this.element.classList.add(this.css.wrapper, this.css.block)
    this.element.contentEditable = 'false'
    this.element.dataset.placeholderActive = this.api.i18n.t(this.placeholder)

    if (this.data.text) {
      this.element.innerHTML = this.data.text
    }

    if (!this.readOnly) {
      this.element.contentEditable = 'true'
      this.element.addEventListener('keyup', this.onKeyUp)
    }

    return this.element
  }

  merge(data: SimpleQuoteData): void {
    if (!this.element) {
      return
    }

    const nextText = normalizeSimpleQuoteText(data.text)

    this.data.text += nextText
    this.element.appendChild(makeFragment(nextText))
    this.element.normalize()
  }

  validate(savedData: SimpleQuoteData): boolean {
    if (savedData.text.trim() === '' && !this.preserveBlank) {
      return false
    }

    return true
  }

  save(toolsContent: HTMLDivElement): SimpleQuoteData {
    return {
      text: normalizeSimpleQuoteText(toolsContent.innerHTML),
    }
  }

  onPaste(event: HTMLPasteEvent): void {
    this.data = {
      text: normalizeSimpleQuoteText(event.detail.data.innerHTML),
    }

    window.requestAnimationFrame(() => {
      if (!this.element) {
        return
      }

      this.element.innerHTML = this.data.text
    })
  }
}
