...
import isPromise from './editorjsImageToolIsPromise'
import type {
  UploadOptions,
...
const IMAGE_ACCEPT =
  'image/png,image/jpeg,image/gif,image/webp,image/svg+xml,image/tiff,.png,.jpg,.jpeg,.gif,.webp,.svg,.tiff'

function selectImageFile(): Promise<File> {
...
export default class Uploader {
  ...
  public uploadSelectedFile({ onPreview }: UploadOptions): void {
    ...
    if (
      ...
      upload = selectImageFile().then((file) => {
        preparePreview(file)
        const customUpload = uploadByFile(file)
    ...
    if (
      ...
      upload = ajax
        .post({
          ...
          data: Object.assign({ url }, this.config.additionalRequestData),
